...
function Popup({lang}) {
    ...
    useEffect(()=>{
        if(pathName.includes('tuyen-dung') || pathName.includes('recruitment')){
            popUpRef.current.style.transform = 'translateY(310%)'
        }else{
    ...
    useEffect
    return (
        <div ref={popUpRef} className={`md:fixed max-md:hidden bottom-[10%] right-[0] z-[9] min-w-[20rem] popUpJob 
        rounded-tl-[1rem] rounded-bl-[1rem] pl-[1rem] pr-[3rem] py-[0.5rem] `}>
