    ...
    <div className='pt-[6.25rem] relative partner'>
      ...
      <div style={{ background: 'linear-gradient(180deg, #FFF 0%, #CFEAE4 100%)' }}>
        ...
        <h3 className='pb-[12.81rem] flex justify-center'>
          <Link className='md:text-[5rem] text-[6.93333rem] max-md:mt-[4rem] text-[#444] font-bold leading-[1.16] text-center tracking-[-0.25rem] title-connect w-max' href={`/${lang}/${lang === 'vi' ? 'lien-he' : 'contact'}/#form_contact`}>{dataPartner?.heading}</Link>
        </h3>
      </div>
    </div>
  )
